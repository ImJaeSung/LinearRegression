...
- EDA
- Early stopping rule 
- smooting loss function : L2, L1 regulazation
- KFold cross-validation : 0.14267 -> 0.13443

#### Invalid metric on test data : R-squared
- SST = $$\sum(y_{i}-\bar{y})^2$$
- SSE = $$\sum({y_{i}}-\hat{y_{i}})^2$$
- SSR = $$\sum(\hat{y_{i}}-\bar{y})^2$$
- SST = SSE + SSR
- But, SST = SSE + SSR is invalid in test data
- SST = SSE + SSR - $$2\sum{e_{i}}(\sum{\hat{y_{i}}}-\bar{y})$$
- Because sum of residual($$\sum{e_{i}}$$) is non zero in test data
- $$R^2 \in (-inf,inf)$$   

#### Classical Linear regression vs DeepLearing
- L : $$\beta$$ $$\mapsto$$ -$$\frac{1}{n} \sum({y-X\hat{\beta}})^2$$
-  min L($$\beta$$)  $$st. \beta\in{\mathbb{R}}^p$$
##### 1. OLSE
- $$e = y-X\hat{\beta} \sim MN(0,\sigma^2I)$$
- $$L(\hat{\beta})=\sum_{i} e_{i}^2 = e^Te = ({y}_{n\times1}-X\hat{\beta})^T({y}_{n\times1}-X\hat{\beta}) = {y}_{n\times1}^T{y}_{n\times1}-2\hat{\beta}^TX^T{y}_{n\times1}+\hat{\beta}^TX^TX\hat{\beta}$$
- $$\frac{\partial L}{\partial \beta} = -2X^T{y}_{n\times1}+2X^TX\hat{\beta}=0$$
- $$\hat{\beta}=(X^TX)^1 X^Ty_{n\times1}$$
